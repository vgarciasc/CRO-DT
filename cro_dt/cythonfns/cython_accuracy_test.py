...
if __name__ == "__main__":
    ...
    for _ in range(100):
        ...
        print("-----------")
        print(f"ACCURACIES: (tree: {acc_tree}, cytree: {acc_cytree}, matrix: {acc_matrix}, cupy: {acc_cupy}, numba: {acc_numba})")

        X = np.vstack((np.ones(len(X)), X.T)).T
        Z = np.sign(W @ X.T)

        # Check if they are all the same
        if acc_tree != acc_matrix or acc_tree != acc_cytree or acc_tree != acc_numba:
            pdb.set_trace()
