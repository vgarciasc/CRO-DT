...
if __name__ == "__main__":
    ...
    command_line += "\n\npython tao.py " + " ".join([f"--{key} {val}" for (key, val) in args.items()]) + "\n\n---\n\n"
    curr_time = datetime.now().strftime("%Y_%m_%d-%H_%M_%S")
    ...
    for dataset in datasets:
        ...
        start_idx = args['start_from'] if dataset == datasets[0] else 0
        ...
        for simulation in simulations:
            ...
            ## fit cart
            # max_rules = 2 ** (args['depth'] + 1) - 1
            # model = GreedyTreeClassifier(max_depth=args['depth'])
            # model.fit(X_train, y_train, feature_names=feat_names)
            # print(model)

            # fit TAO
            should_print_model = True
            model = TaoTreeClassifier(n_iters=100, model_args={'max_depth': args['depth']}, verbose=should_print_model)
            model.fit(X_train, y_train, feature_names=feat_names)
            ...
            n_nodes = model.model.get_n_leaves() * 2 - 1
            ...
            if args["should_save"]:
                save_log_to_file(f"log_tao_{curr_time}.txt", log, command_line)
